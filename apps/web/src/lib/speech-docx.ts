...
// Word and LibreOffice do not receive the webfont bundled by the app. Arial
// is present on the supported desktop targets and includes Cyrillic glyphs.
...
function paragraph(text: string, style?: string) {
  ...
  return `<w:p>${styleXml}<w:r><w:rPr>${docxFontProperties()}</w:rPr><w:t xml:space="preserve">${escapeXml(text.trim())}</w:t></w:r></w:p>`;
}

function docxFontProperties() {
  return `<w:rFonts w:ascii="${DOCX_FONT_FAMILY}" w:hAnsi="${DOCX_FONT_FAMILY}" w:eastAsia="${DOCX_FONT_FAMILY}" w:cs="${DOCX_FONT_FAMILY}"/><w:lang w:val="ru-RU"/>`;
}
