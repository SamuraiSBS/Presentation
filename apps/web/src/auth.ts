  ...
  events: {
    async signIn({ user, account, profile }) {
      ...
      await prisma.user.update({
        where: { id: user.id },
        data: {
          // Telegram OIDC `sub` is a long OIDC subject identifier. The
          // numeric Telegram account ID used by ADMIN_TELEGRAM_IDS is `id`.
          telegramId: String(telegramProfile.id ?? telegramProfile.sub),
