    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="share-dialog">
        ...
        <section className="invite-builder" aria-labelledby="invite-builder-title">
          ...
          <div className="invite-controls"><select className="input" value={role} onChange={(event) => setRole(event.target.value as ProjectMemberRole)}><option value="viewer">Только просмотр и экспорт</option><option value="editor">Редактирование и экспорт</option></select><button className="button" type="button" onClick={() => void createLink()} disabled={createInvitation.isPending}>{createInvitation.isPending ? <LoaderCircle className="spin" size={18} /> : <Link2 size={18} />}Создать ссылку</button></div>
        ...
        <section className="members-section" aria-labelledby="members-title">
          ...
          {membersQuery.data?.members.length ? <div className="members-list">{membersQuery.data.members.map((member) => <div className="member-row" key={member.id}><span className="member-avatar">{member.user.image ? <img src={member.user.image} alt="" /> : <UserRound size={17} />}</span><div><strong>{member.user.name || "Участник"}</strong><small>{member.user.telegramUsername ? `@${member.user.telegramUsername}` : "Telegram"}</small></div><select aria-label={`Роль: ${member.user.name || "участник"}`} value={member.role} onChange={(event) => void updateMember.mutateAsync({ memberId: member.id, role: event.target.value as ProjectMemberRole })}><option value="viewer">Просмотр</option><option value="editor">Редактор</option></select><button className="icon danger-icon" type="button" aria-label="Отозвать доступ" onClick={() => void removeMember.mutateAsync(member.id)}><Trash2 size={16} /></button></div>)}</div> : !membersQuery.isLoading ? <p className="muted">Пока доступ есть только у вас.</p> : null}
