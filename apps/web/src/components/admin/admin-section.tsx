...
function EnvelopeTable({ envelopes }: { envelopes: Array<Record<string, unknown>> }) {
  ...
  return <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Economic run</th><th>Бюджет</th><th>Модели / Tavily</th><th>Резервы</th><th>Остановка</th></tr></thead><tbody>{envelopes.map((envelope) => {
    ...
    return <tr key={String(envelope.id)}><td data-label="Economic run"><strong>{String(envelope.id)}</strong><small>{String(envelope.projectId)} · {String(envelope.status)}</small></td><td data-label="Бюджет"><Money value={String(envelope.settledRub)} /> / <Money value={String(envelope.limitRub)} /><small>Остаток: <Money value={String(envelope.remainingRub)} /></small></td><td data-label="Модели / Tavily">{((envelope.actualModels || []) as string[]).join(", ") || "AI-вызовов нет"}<small>Tavily queries: {String(envelope.tavilyQueries)}</small></td><td data-label="Резервы">{reservations.map((item) => <small key={String(item.id)}>{String(item.stage)}: {String(item.status)} · reserved {String(item.reservedRub)} · settled {String(item.settledRub)}</small>)}</td><td data-label="Остановка">{envelope.terminationReason ? String(envelope.terminationReason) : "—"}</td></tr>;
  })}</tbody></table></div>;
}
...
function RecordTable({ items, section, onAction }: { items: Array<Record<string, unknown>>; section: string; onAction?: (path: string, body?: unknown) => void }) {
  ...
  return <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Объект</th><th>Статус / тип</th><th>Контекст</th><th>Сумма</th><th>Время</th>{section === "generations" ? <th>Действия</th> : null}</tr></thead><tbody>{items.map((item) => <tr key={String(item.id)}><td data-label="Объект"><strong>{String(item.projectTitle || item.message || item.action || item.provider || item.id)}</strong><small>{String(item.error || item.fingerprint || item.targetId || "")}</small></td><td data-label="Статус / тип">{String(item.status || item.severity || item.type || item.category || item.measurement || "—")}</td><td data-label="Контекст">{String(item.model || item.service || item.userId || item.currency || "—")}</td><td data-label="Сумма">{item.rubCostAtEvent != null || item.netRubAtEvent != null ? <Money value={String(item.rubCostAtEvent || item.netRubAtEvent)} /> : <span className="admin-muted">—</span>}</td><td data-label="Время"><DateTime value={String(item.updatedAt || item.occurredAt || item.createdAt || "")} /></td>{section === "generations" ? <td data-label="Действия"><div className="admin-row-actions">{item.status === "failed" ? <button className="ghost" onClick={() => onAction?.(`generations/${String(item.id)}/retry`)}>Повторить</button> : null}{item.status === "queued" || item.status === "active" ? <button className="admin-danger-button" onClick={() => { const reason = window.prompt("Причина отмены"); if (reason) onAction?.(`generations/${String(item.id)}/cancel`, { reason }); }}>Отменить</button> : null}</div></td> : null}</tr>)}</tbody></table></div>;
}
