...
export function AdminUsers() {
  ...
  return <section className="admin-page"><header className="admin-page-heading"><div><h1>Пользователи</h1><p>Профили и экономика без промптов, исходных материалов и содержимого презентаций.</p></div></header>
    ...
    <div className="admin-table-wrap">{data.items.length ? <table className="admin-table"><thead><tr><th>Пользователь</th><th>Тариф</th><th>Активность</th><th>Проекты / генерации</th><th>Расход</th><th>Выручка</th><th>Ошибки</th><th>Статус</th></tr></thead><tbody>{data.items.map((user) => <tr key={user.id}><td><Link className="admin-user-link" href={`/admin/users/${user.id}`}><span className="admin-avatar">{user.name?.slice(0, 1) || "?"}</span><span><strong>{user.name || "Без имени"}</strong><small>@{user.telegramUsername || "—"} · {user.telegramId || user.id}</small></span></Link></td><td><span className="admin-plan">{user.effectivePlanCode}</span><small>{user.subscriptionStatus || "без подписки"}</small></td><td><DateTime value={user.lastSeenAt} /><small>Регистрация: <DateTime value={user.createdAt} /></small></td><td>{user.projects} / {user.generations}</td><td><Money value={user.totalCostRub} unknown={user.totalCostRub === null} /></td><td><Money value={user.revenueRub} /></td><td>{user.errors}</td><td><span className={user.blockedAt ? "admin-status admin-status-danger" : "admin-status admin-status-ok"}>{user.blockedAt ? "Заблокирован" : "Активен"}</span></td></tr>)}</tbody></table> : <AdminEmpty title="Пользователи не найдены" body="Очистите строку поиска или измените фильтры." />}</div>
