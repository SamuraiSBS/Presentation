...
**Статус:** source build, actual bundle gate, visual smoke доступных routes и textarea fallback восстановлены. Local production acceptance не принято из-за красного Lighthouse, непроверенного content-state defense demo и зависания Docker/BuildKit image build.

**Нужно доделать:**

- Устранить Lighthouse regression production output (последний прогон: score 43, LCP 5062 ms, TBT 666 ms), затем повторить `npm run lighthouse:production -- --assert` на exact Docker image; не менять Lighthouse budgets без принятого evidence.
- Восстановить Docker Desktop/BuildKit на хосте: один plain `Dockerfile.web` build завис до первой строки progress. После восстановления собрать exact image, запустить его и повторить Lighthouse/CWV smoke.
- Добавить или выбрать demo fixture с defense workspace, затем принять visual content-state `/projects/<id>/defense/{plan,review}`; текущий `demo` покрывает route error-state, но не defense workflow content.
